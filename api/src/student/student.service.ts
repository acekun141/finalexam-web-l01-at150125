...
import { UserWithUsernameAlreadyExisted } from "../_exception";
import { v4 } from "uuid";
...
export default class StudentService {
  ...
  public createStudent = async (
    data: Omit<IUserInfo, "avatar"> & Omit<User, "id" | "role">, user: User
  ) => {
		...
		try {
			...
			const student = await this.userModel.create({ username, password, role: "student", id });
			await this.userInfoModel.create({ user_id: id, first_name, last_name, sex, date_of_birth });
			const relationship = this.relationshipModel.findOne({ parent_id: user.id });
			if (!relationship) {
	...
	public getListChildrenWithParent = async (parent_id: string) => {
		try {
			const relationship = await this.relationshipModel.findOne({ parent_id }).populate("children", "id");
			console.log(relationship);
			const listChildren = await Promise.all(relationship.children.map(async (item: any) => {
				const childrenInfo = await this.userInfoModel.findOne({ user_id: item.id });
				return { first_name: childrenInfo.first_name, last_name: childrenInfo.last_name, id: childrenInfo.user_id, sex: childrenInfo.sex, date_of_birth: childrenInfo.date_of_birth };
			}));
			return listChildren;
		} catch(error) {
			throw error;
